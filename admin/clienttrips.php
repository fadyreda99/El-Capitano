...
if (isset($_SESSION['admin'])) {
    ...
    if ($do == 'manage') {
        ...
        $trips = $tripObject->all("UserID = {$userid} ORDER BY Trip_Date");


        if (!empty($trips)) {
?>
            <h1 class="text-center edit">client trips</h1>
            <div class="container ">
                <div class="table-responsive trips-table">
                    ...
                    <div class="row text-center " style="width: 100%;">
                        <div class="col-4">
                            <a class="btn btn-primary " href="clienttrips.php?do=unfinish&userid=<?php echo $trip['UserID'] ?>">unfinished trips</a>

                        </div>
                        <div class="col-4">
                            <a class="btn btn-success" href="clienttrips.php?do=finished&userid=<?php echo $trip['UserID'] ?>">finished trips</a>

                        </div>
                        <div class="col-4">
                            <a class="btn btn-danger" href="clienttrips.php?do=cancelled&userid=<?php echo $trip['UserID'] ?>">cancelled trips</a>
        ...
        $userid = isset($_GET['userid']) && is_numeric($_GET['userid']) ? intval($_GET['userid']) : 0;
        $trips = $tripObject->all("UserID = {$userid} AND Status = 0 ORDER BY Trip_Date");
        if (!empty($trips)) {
        ?>
        ...
        $userid = isset($_GET['userid']) && is_numeric($_GET['userid']) ? intval($_GET['userid']) : 0;
        $trips = $tripObject->all("UserID = {$userid} AND Status = 1 ORDER BY Trip_Date");
        ...
        $userid = isset($_GET['userid']) && is_numeric($_GET['userid']) ? intval($_GET['userid']) : 0;
        $trips = $tripObject->all("UserID = {$userid} AND Status = 2 ORDER BY Trip_Date");
